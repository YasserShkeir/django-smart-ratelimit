...
class RedisBackend(BaseBackend):
    ...
    # Lua script for fixed window rate limiting (simpler, more memory efficient)
    FIXED_WINDOW_SCRIPT = """
        local key = KEYS[1]
        local window = tonumber(ARGV[1])
        local limit = tonumber(ARGV[2])
    ...
    def __init__(self):
        ...
        # Configuration
        self.use_sliding_window = getattr(settings, 'RATELIMIT_USE_SLIDING_WINDOW', True)
    ...
    def incr(self, key: str, period: int) -> int:
        ...
        Uses either sliding window or fixed window algorithm based on configuration.
        """
    ...
    def get_count(self, key: str) -> int:
        ...
        if self.use_sliding_window:
            ...
            now = time.time()
            # We don't know the window size here, so we'll return the total count
