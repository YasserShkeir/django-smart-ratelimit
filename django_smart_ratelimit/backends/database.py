...
from ..models import RateLimitEntry, RateLimitCounter
...
class DatabaseBackend(BaseBackend):
    ...
    def _incr_fixed_window(self, key: str, window_seconds: int) -> int:
        ...
        with transaction.atomic():
            ...
            if not created and (counter.window_start != window_start or counter.window_end != window_end):
                counter.count = 0
    ...
    def _get_count_fixed_window(self, key: str, window_seconds: int) -> int:
        ...
        try:
            ...
            if counter.window_start == window_start and counter.window_end == window_end:
                return counter.count
            else:
    ...
    def _get_reset_time_sliding_window(self, key: str, window_seconds: int) -> Optional[int]:
        """Get reset time for sliding window algorithm."""
        # For sliding window, find the oldest entry
        oldest_entry = RateLimitEntry.objects.filter(key=key).order_by('timestamp').first()
    ...
    def _get_reset_time_fixed_window(self, key: str, window_seconds: int) -> Optional[int]:
    ...
    def incr_with_algorithm(self, key: str, window_seconds: int, algorithm: str = "sliding_window") -> int:
        """
    ...
    def get_count_with_algorithm(self, key: str, window_seconds: int, algorithm: str = "sliding_window") -> int:
        """
    ...
    def get_reset_time_with_algorithm(self, key: str, window_seconds: int, algorithm: str = "sliding_window") -> Optional[int]:
        """
    ...
    def reset_with_algorithm(self, key: str, window_seconds: int, algorithm: str = "sliding_window") -> bool:
        """
    ...
    def _cleanup_expired_entries(self, force: bool = False) -> int:
        ...
        if not force:
            ...
            total_entries = RateLimitEntry.objects.count() + RateLimitCounter.objects.count()
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            'healthy': healthy,
            'backend': 'database',
            'message': 'Database health check completed' if healthy else 'Database health check failed',
            'details': details
        }
