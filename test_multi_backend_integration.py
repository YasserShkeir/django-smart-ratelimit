...
def test_multi_backend_integration():
    ...
    try:
        ...
        settings.RATELIMIT_MULTI_BACKEND_STRATEGY = test_settings['RATELIMIT_MULTI_BACKEND_STRATEGY']
        settings.RATELIMIT_HEALTH_CHECK_INTERVAL = test_settings['RATELIMIT_HEALTH_CHECK_INTERVAL']
        
        # Get multi-backend
        backend = get_backend()
        
        # Verify it's a MultiBackend
        assert isinstance(backend, MultiBackend), f"Expected MultiBackend, got {type(backend)}"
        ...
        assert stats['total_backends'] == 2, f"Expected 2 backends, got {stats['total_backends']}"
        assert stats['healthy_backends'] == 2, f"Expected 2 healthy backends, got {stats['healthy_backends']}"
        assert stats['fallback_strategy'] == 'first_healthy', f"Expected first_healthy strategy, got {stats['fallback_strategy']}"
