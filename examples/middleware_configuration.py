...
ADVANCED_MIDDLEWARE_CONFIG = {
    ...
    "RATELIMIT_PATTERNS": [
        ...
        },
        {
            ...
            "key": "user",
            "backend": "redis_primary",
            "skip_if": lambda request: not request.user.is_authenticated,
        },
        {
            "pattern": r"^/api/admin/",
            "rate": "10000/h",
            "key": "user",
            "backend": "redis_primary",
            "skip_if": lambda request: not request.user.is_staff,
        },
        {
    ...
    "RATELIMIT_GLOBAL": {
        ...
        "skip_if": lambda request: request.META.get("HTTP_X_BYPASS_RATELIMIT")
        == "secret_key",
    },
}
...
class CustomRateLimitMiddleware(RateLimitMiddleware):
    ...
    def rate_limit_exceeded_response(self, request, rate_limit_info):
        ...
        if rate_limit_info.get('consecutive_violations', 0) > 5:
            self.send_abuse_notification(request, rate_limit_info)
    ...
    def send_abuse_notification(self, request, rate_limit_info):
...
URL_SPECIFIC_CONFIG = {
    "RATELIMIT_PATTERNS": [
        ...
        # File uploads - resource intensive
        {
            ...
            "rate": "10/h",
            "key": lambda request: f"user:{request.user.id}"
...
ALGORITHM_AWARE_MIDDLEWARE_CONFIG = {
    ...
    "RATELIMIT_MIDDLEWARE": {
        ...
        "SKIP_IF": lambda request: (
            ...
            or request.META.get("HTTP_X_BYPASS_RATELIMIT") == "true"
        ),
        ...
        "RATE_LIMITS": {
            ...
            "/api/auth/": {
                ...
                "skip_if": lambda request: request.method == "GET",
            },
        },
...
def smart_key_function(request):
    """
    Custom key function that considers user type and request characteristics.
    """
...
CUSTOM_KEY_MIDDLEWARE_CONFIG = {
    "RATELIMIT_MIDDLEWARE": {
        ...
        "SKIP_IF": lambda request: smart_key_function(request) is None,
    }
}
