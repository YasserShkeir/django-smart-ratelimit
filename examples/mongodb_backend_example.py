...
def example_basic_usage():
    ...
    try:
        ...
        print(f"First request count: {count1}")

        count2 = backend.incr(test_key, 60)
        print(f"Second request count: {count2}")
...
def example_atlas_configuration():
...
def example_advanced_decorator_usage():
    ...
    @ratelimit(
        ...
        backend="mongodb",
        algorithm="sliding_window",
        skip_if=lambda request: request.META.get("REMOTE_ADDR", "").startswith("127."),
    )
    def mongodb_api_with_algorithm(request):
        """
    ...
    def mongodb_user_api_with_bypass(request):
        """
    ...
    try:
        ...
        print(f"User request (non-staff): Status {response2.status_code}")

        response3 = mongodb_user_api_with_bypass(request3)
        print(f"User request (staff): Status {response3.status_code}")
