...
def rate_limit_metrics(request):
    """
...
def monitoring_key_function(request):
...
def monitored_api(request):
    ...
    endpoint_metrics_key = f"endpoint_metrics:{request.path}"
...
def alerting_key_function(request):
...
def alerting_api(request):
    ...
    """
    backend = get_backend()
    key = alerting_key_function(request)
...
def performance_monitoring_key(request):
...
def performance_monitored_api(request):
    """
    ...
    try:
        backend = get_backend()
        key = performance_monitoring_key(request)
