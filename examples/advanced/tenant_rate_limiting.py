...
@rate_limit(key=tenant_aware_key, rate="500/h")
def tenant_api(_request):
    """
...
def hierarchical_tenant_key(_request):
    """
    ...
    """
    org_id = _request.headers.get("X-Organization-ID")
    tenant_id = _request.headers.get("X-Tenant-ID")

    if org_id and tenant_id:
        if is_authenticated_user(_request):
            return f"org:{org_id}:tenant:{tenant_id}:user:{_request.user.id}"
        else:
            return f"org:{org_id}:tenant:{tenant_id}"
    elif tenant_id:
        return f"tenant:{tenant_id}"
    elif is_authenticated_user(_request):
...
def hierarchical_tenant_api(_request):
    """
    ...
    return JsonResponse(
        {
            ...
            "tenant_id": tenant_id,
            "user_id": _request.user.id if is_authenticated_user(_request) else None,
...
def tenant_feature_key(_request):
    """
    ...
    """
    tenant_id = _request.headers.get("X-Tenant-ID")
    feature = _request.headers.get("X-Feature", "default")

    if tenant_id:
        return f"tenant:{tenant_id}:feature:{feature}"
    else:
        return f"ip:{_request.META.get('REMOTE_ADDR', 'unknown')}:feature:{feature}"


@rate_limit(key=tenant_feature_key, rate="100/h")
def tenant_feature_api(_request):
    """
...
def tenant_api_key_function(_request):
    """
...
def tenant_api_key_endpoint(_request):
    """
...
def tenant_subdomain_key(_request):
    """
...
def tenant_subdomain_api(_request):
    """
...
def tenant_usage_tracking(_request):
    """
    ...
    return process_request(_request)


# Example: Tenant rate limiting with algorithm and skip_if
@rate_limit(
    key=tenant_aware_key,
    rate="500/h",
    algorithm="sliding_window",
    skip_if=lambda _request: _request.headers.get("X-Tenant-ID") == "premium-tenant",
)
def tenant_api_advanced(_request):
    """
...
@rate_limit(
    ...
    skip_if=lambda _request: _request.headers.get("X-Organization-ID")
    == "enterprise-org",
)
def hierarchical_tenant_api_advanced(_request):
    """
    ...
    return JsonResponse(
        {
            ...
            "organization_id": org_id,
            "tenant_id": tenant_id,
            "user_id": _request.user.id if _request.user.is_authenticated else None,
            "algorithm": "fixed_window",
