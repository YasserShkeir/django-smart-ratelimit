...
class DRFIntegrationTestCase(APITestCase):
    ...
    def test_viewset_rate_limiting(self):
        ...
        from examples.integrations.drf_integration.viewsets import PostViewSet

        # Create a mock _request
        _request = self.factory.get("/api/posts/")
        _request.user = self.user

        # Test the viewset
        viewset = PostViewSet()
        viewset.action = "list"
        viewset._request = _request
        viewset.format_kwarg = None

        # Test list action - the decorator should be applied at import time
        response = viewset.list(_request)
        ...
        # Test other methods
        response = viewset.retrieve(_request, pk=1)
        self.assertEqual(response.status_code, 200)
    ...
    def test_serializer_integration(self):
        ...
        class SimplePostSerializer(serializers.Serializer):
            """SimplePostSerializer implementation."""
        ...
        # Create serializer with _request context
        _request = self.factory.post("/api/posts/", data)
        _request.user = self.user

        serializer = SimplePostSerializer(data=data, context={"_request": _request})
        ...
        invalid_serializer = SimplePostSerializer(
            data=invalid_data, context={"_request": _request}
        )
    ...
    def test_permission_integration(self):
        ...
        from examples.integrations.drf_integration.permissions import (
        ...
        with patch.object(permission, "_check_rate_limit", return_value=True):
            result = permission.has_permission(_request, view)
            self.assertTrue(result)

        # Test rate limit exceeded
        with patch.object(permission, "_check_rate_limit", return_value=False):
            with self.assertRaises(Throttled):
                permission.has_permission(_request, view)
...
class DRFIntegrationUnitTests(TestCase):
    ...
    def test_mock_post_model(self):
        ...
        from examples.integrations.drf_integration.viewsets import Post
    ...
    def test_mock_comment_model(self):
        ...
        from examples.integrations.drf_integration.viewsets import Comment
