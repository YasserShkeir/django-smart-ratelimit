...
from django_smart_ratelimit import rate_limit, is_authenticated_user, user_or_ip_key
...
from django_smart_ratelimit import rate_limit, is_authenticated_user, user_or_ip_key, tenant_aware_key
...
class AuthenticatedAPIView(APIView):
    ...
    def get_rate_limit_key(self, _request):
        ...
        return f"ip:{_request.META.get('REMOTE_ADDR')}"
...
class TenantAPIView(APIView):
    ...
    def get_tenant_key(self, _request):
        tenant_id = _request.headers.get('X-Tenant-ID')
        if tenant_id:
            return f"tenant:{tenant_id}"
        return f"user:{_request.user.id}" if is_authenticated_user(_request) else f"ip:{_request.META.get('REMOTE_ADDR')}"
...
def drf_rate_limit_response(_request):
...
def rate_limited_drf_view(_request):
...
class RateLimitedAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, _request, extra_context=None):
        # Rate limit bulk operations
        return super().changelist_view(_request, extra_context)

    @rate_limit(key='user', rate='5/h')
    def delete_view(self, _request, object_id, extra_context=None):
        # Rate limit deletions
        return super().delete_view(_request, object_id, extra_context)

# Custom admin action with rate limiting
@rate_limit(key='user', rate='1/h')
def bulk_export_action(modeladmin, _request, queryset):
...
class RateLimitedForm(forms.Form):
    ...
    def __init__(self, *_args, **_kwargs):
        self._request = _kwargs.pop('_request', None)
        super().__init__(*_args, **_kwargs)
        self.rate_limit_backend = get_backend('redis')
...
class APIKeyRateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, _request):
        ...
        if not api_key:
            ...
            key = f"no_api_key:{_request.META.get('REMOTE_ADDR')}"
...
class GeographicRateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, _request):
        ...
        client_ip = _request.META.get('REMOTE_ADDR')
