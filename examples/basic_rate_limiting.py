...
def api_combined_limits(request):
    ...
    """
    user_info = (
        "anonymous" if not request.user.is_authenticated else f"user_{request.user.id}"
    )
...
def api_skip_for_staff(request):
    ...
    """
    user_type = "staff" if request.user.is_staff else "regular user"
    rate_limited = "No" if request.user.is_staff else "Yes (10/min)"
...
@rate_limit(key="ip", rate="5/m", block=custom_rate_limit_response)
def api_custom_error(request):
    """
